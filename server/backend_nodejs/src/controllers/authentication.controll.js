...
const asyncHandler = require("@src/services/asyncHandler")
...
const {ResponeAuthentication} = require("@src/models/reponse.model")
...
const loginController = asyncHandler(async (req, res, next) => {
    login(req.body, (error, result) => {
        ...
        return res.status(200).json(
            new ResponeAuthentication({accesstoken,refreshtoken,message:LOGIN_SUCCESSFULL})
        )
    })
})
...
const registerController = asyncHandler(async (req, res, next) => {
    register(req.body, (error, result) => {
        ...
        return res.status(201).json(
            new ResponeAuthentication({accesstoken,refreshtoken,message:REGISTER_SUCCESSFULL})
        )
    })
})
